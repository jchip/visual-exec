...
class VisualExec {
  ...
  _updateDigest(item, buf) {
    ...
    const lines = newBuf
      ...
      .map(x => {
        x = x && x.trim();
        return x && hasAnsi(x) ? stripAnsi(x) : x;
      })
      .filter(x => x);

    let length = 0;
    let ix = lines.length - 1;
    for (; ix >= 0; ix--) {
      const line = lines[ix];
      if (line) {
    ...
    if (msgs.length === 0) {
      // even the last line is too long, save it, and truncate it for display
      item.buf = lines[lines.length - 1];
      msgs = [item.buf.substr(0, 100)];
    } else {
      item.buf = msgs.join("\n");
    }
